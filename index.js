...
app.post("/api/daily-entry", async (req, res) => {
  const { entry, water, smoke } = req.body;
  ...
  async function main() {
    try {
      ...
      const insertQuery = `
        INSERT INTO stats (date, health, energy, mental, charisma, intellect, skill, water, smoke, journal_entry, embedding)
  ...
  try {
    ...
    if (streakType === 'porn') {
      if (reset) {
        ...
        // Automatically increment porn streak daily
        updateQuery = `
          INSERT INTO stats (date, porn_streak)
          VALUES (CURRENT_DATE, 
            CASE 
              WHEN EXISTS (SELECT 1 FROM stats WHERE date = CURRENT_DATE - INTERVAL '1 day')
      ...
      if (reset) {
        ...
        updateQuery = `
          ...
          ON CONFLICT (date) 
          DO UPDATE SET workout_streak = 
            CASE
              WHEN stats.workout_streak = 0 THEN 1
              ELSE stats.workout_streak + 1
            END
          RETURNING workout_streak;
        `;
      }
