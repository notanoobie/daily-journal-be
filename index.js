...
app.post("/api/daily-entry", async (req, res) => {
  ...
  try {
    ...
    // Update database with all data including Google Fit metrics
    const insertQuery = `
      INSERT INTO stats (
...
async function getGoogleFitData() {
  try {
    ...
    // Separate requests for each data type to handle permissions individually
    ...
    return {
      steps: responses[0].status === 'fulfilled' ? (responses[0].value.data.bucket[0]?.dataset[0]?.point[0]?.value[0]?.intVal || 0) : 0,
      calories: responses[1].status === 'fulfilled' ? (responses[1].value.data.bucket[0]?.dataset[0]?.point[0]?.value[0]?.fpVal || 0) : 0,
      activeMinutes: responses[2].status === 'fulfilled' ? (responses[2].value.data.bucket[0]?.dataset[0]?.point[0]?.value[0]?.intVal || 0) : 0
    };
  } catch (error) {
