...
async function getGoogleFitData() {
  try {
    ...
    const responses = await Promise.allSettled([
      ...
      // Oxygen saturation data - updated
      fitness.users.dataset.aggregate({
        userId: 'me',
        requestBody: {
          aggregateBy: [{ 
            dataTypeName: 'com.google.oxygen_saturation.percentage',
            dataSourceId: 'derived:com.google.oxygen_saturation.percentage:com.google.android.gms:merged'
          }],
          bucketByTime: { durationMillis: 86400000 },
          startTimeMillis: yesterday,
          endTimeMillis: now
      ...
      fitness.users.dataset.aggregate({
        ...
        requestBody: {
          ...
          bucketByTime: { durationMillis: 86400000 * 2 }, // Look back 48 hours for sleep
          startTimeMillis: yesterday - (24 * 60 * 60 * 1000), // Look back an extra day for sleep
          endTimeMillis: now
        }
      })
    ]);
    ...
    responses.forEach((response, index) => {
      if (response.status === 'rejected') {
        console.error(`Request ${index} failed:`, {
          ...
          details: response.reason.response?.data?.error
        });
      } else {
        console.log(`Request ${index} succeeded with data points:`, 
          response.value.data.bucket.reduce((count, bucket) => 
            count + bucket.dataset.reduce((c, ds) => 
              c + (ds.point?.length || 0), 0), 0)
        );
      }
    });

    // Process heart rate data - Update the processing since we're getting data
    const heartRateData = responses[1].status === 'fulfilled' ? 
      responses[1].value.data.bucket[0]?.dataset[0]?.point || [] : [];
    const heartRate = heartRateData.length ? {
      avg: Math.round(heartRateData[0].value[0].fpVal), // First value is average
      max: Math.round(heartRateData[0].value[1].fpVal), // Second value is max
      min: Math.round(heartRateData[0].value[2].fpVal)  // Third value is min
    } : { avg: 0, min: 0, max: 0 };

    // For oxygen, let's try a different data type name
    fitness.users.dataset.aggregate({
      userId: 'me',
      requestBody: {
        aggregateBy: [{ 
          dataTypeName: 'com.google.blood_oxygen',
          dataSourceId: 'derived:com.google.blood_oxygen:com.google.android.gms:merged'
        }],
        bucketByTime: { durationMillis: 86400000 },
        startTimeMillis: yesterday,
        endTimeMillis: now
    ...
    // For sleep, let's try a different approach
    fitness.users.dataset.aggregate({
      userId: 'me',
      requestBody: {
        aggregateBy: [{ 
          dataTypeName: 'com.google.sleep.segment',
          dataSourceId: 'derived:com.google.sleep.segment:com.google.android.gms:sleep_from_activity<-raw:com.google.activity.segment:com.coveiot.android.boat:GoogleFitDataManager'
        }],
        bucketByTime: { durationMillis: 86400000 * 2 },
        startTimeMillis: yesterday - (24 * 60 * 60 * 1000),
        endTimeMillis: now
      }
    })
    ...
    console.log('Available data sources:', JSON.stringify(dataSources.data, null, 2));
