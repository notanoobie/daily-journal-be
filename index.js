...
app.post("/api/daily-entry", async (req, res) => {
  ...
  async function main() {
    try {
      ...
      // Update porn streak
      const pornStreakQuery = `
        INSERT INTO stats (date, porn_streak)
        VALUES (CURRENT_DATE, 
          CASE 
            WHEN EXISTS (SELECT 1 FROM stats WHERE date = CURRENT_DATE - INTERVAL '1 day' AND porn_streak > 0)
            THEN (SELECT porn_streak + 1 FROM stats WHERE date = CURRENT_DATE - INTERVAL '1 day')
            ELSE 1
          END
        )
        ON CONFLICT (date) 
        DO UPDATE SET porn_streak = EXCLUDED.porn_streak
        RETURNING porn_streak;
      `;
      const pornStreakResult = await client.query(pornStreakQuery);
      const newPornStreak = pornStreakResult.rows[0].porn_streak;
 
      // Update workout streak only if workout is true
      let newWorkoutStreak = 0;
      if (workout) {
        const workoutStreakQuery = `
          UPDATE stats
          SET workout_streak = workout_streak + 1
          WHERE date = CURRENT_DATE
          RETURNING workout_streak;
        `;
        const workoutStreakResult = await client.query(workoutStreakQuery);
        newWorkoutStreak = workoutStreakResult.rows[0].workout_streak;
      }
...
// Endpoint to update streaks
app.post("/api/update-streak", async (req, res) => {
  const { streakType, reset } = req.body;
  ...
  try {
    ...
    if (streakType === 'porn') {
      if (reset) {
        ...
        updateQuery = `
          ...
          ON CONFLICT (date) 
          DO UPDATE SET porn_streak = stats.porn_streak + 1
          RETURNING porn_streak;
        `;
      }
      ...
      if (reset) {
        ...
        updateQuery = `
          INSERT INTO stats (date, workout_streak)
          VALUES (CURRENT_DATE, 1)
          ON CONFLICT (date) 
          DO UPDATE SET workout_streak = stats.workout_streak + 1
          RETURNING workout_streak;
        `;
      }
    } else {
    ...
    res.status(200).json({ [streakType + '_streak']: result.rows[0][streakType + '_streak'] });
  } catch (error) {
